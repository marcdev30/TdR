module.exports = () => {
  ...
  router.get("/temps", async (req, res) => {
    ...
    let tempsObjectiuAbans = new Date(
      tempsActual.getFullYear(),
      tempsActual.getMonth(),
      tempsActual.getDate(),
      8,
    ...
    let diferenciaTempsMin = tempsObjectiuAbans - tempsActual;
    if (diferenciaTempsMin < 0 && diferenciaTempsMs > 0) {
      obert = true;
    }
    ...
    const minuts = Math.floor(
      (diferenciaTempsMs % (60 * 60 * 1000)) / (60 * 1000)
    );
    const segons = Math.floor((diferenciaTempsMs % (60 * 1000)) / 1000);
    res.json({ obert, hores: hores, minuts: minuts, segons: segons });
  });
